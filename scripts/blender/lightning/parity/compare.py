...
def main():
    ...
    inside = coverage[..., 0] > 0.5
    report("coverage", np.abs(engine_display(coverage[..., 0]) - engine_coverage), inside)

    composited = color[..., :3] + VIEWPORT_BACKGROUND_LINEAR
    report("color", np.abs(engine_display(composited) - engine_color).mean(axis=2), inside)

    ys, xs = np.where(coverage[..., 0] > 0)
