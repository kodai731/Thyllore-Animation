...
use thyllore_shader_manifest::{
    collect_shader_sources, collect_spirv_files, find_declared_block, generate_gpu_blocks_rust,
    generate_pass_manifest_rust, generate_shader_bindings_rust, shader_entries, slang_root,
    spirv_command, spirv_output_name, spirv_rerun_if_env_changed, verify_slangc_version,
    EntryPoint, GpuBlockCodegenConfig, PassManifest, ShaderSource,
};
...
fn gpu_block_source(reflections: &BTreeMap<String, ShaderReflection>, block_name: &str) -> String {
    ...
    let config = GpuBlockCodegenConfig {
        header: format!("Generated by build.rs from the SPIR-V of {first_name}; do not edit."),
        imports: vec!["cgmath::Matrix4".into()],
        extra_derives: BTreeMap::new(),
    };
    generate_gpu_blocks_rust(&block, &config).unwrap_or_else(|error| {
