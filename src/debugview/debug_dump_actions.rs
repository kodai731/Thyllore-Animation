...
pub(crate) fn save_water_probe_if_requested(app: &mut App) {
    ...
    let water_probe_path = match dump_plan.and_then(|plan| plan.water_probe_path.clone()) {
        Some(p) => p,
        None => return,
    };
    ...
    let slice = unsafe { std::slice::from_raw_parts(data_ptr as *const u8, image_size as usize) };
    ...
    let inv_view_proj =
        crate::ecs::systems::water::probe::inverse_view_proj_f64(proj_data.proj, proj_data.view);
