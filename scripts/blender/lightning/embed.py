...
def add_lightning():
    existing = [o.name for o in bpy.context.scene.objects if o.thyllore_lightning.is_lightning]
    if existing:
        print(f"[lightning/embed] scene already holds lightning {existing}", flush=True)
        return
    if bpy.context.mode != "OBJECT":
        bpy.ops.object.mode_set(mode="OBJECT")
    bpy.ops.thyllore.lightning_add()
    print(f"[lightning/embed] added {bpy.context.active_object.name}", flush=True)
...
def orbit_view_once():
    ...
    print("[lightning/embed] orbited view to check stale-frame clearing", flush=True)
    return None


def take_screenshot():
    path = str(Path(SCREENSHOT_DIR) / "lightning_viewport.png")
    found = find_view3d_region()
    if found is None:
        print("[lightning/embed] no VIEW_3D area for screenshot", flush=True)
    else:
        window, area, region = found
        with bpy.context.temp_override(window=window, area=area, region=region):
            bpy.ops.screen.screenshot_area(filepath=path)
            bpy.ops.thyllore.lightning_dump_debug(out_dir=SCREENSHOT_DIR)
        print(f"[lightning/embed] screenshot -> {path}", flush=True)
