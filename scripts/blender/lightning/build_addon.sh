...
SOURCE_DIR="$REPO_ROOT/blender_addon/effects/lightning"
...
Build the Thyllore Lightning extension ZIP for the requested platform.
...
Options:
  --build-mode MODE     release (default) strips the debug/ package;
                        debug keeps it and names the ZIP thyllore_lightning_debug-*
...
case "$BUILD_MODE" in
    release) ZIP_BASENAME="thyllore_lightning" ;;
    debug)   ZIP_BASENAME="thyllore_lightning_debug" ;;
...
log() { echo "[lightning/build_addon] $*"; }

log "Platform: $PLATFORM -> Blender: $BLENDER_NAME, mode: $BUILD_MODE, version: $VERSION"

log "Exporting lightning GLSL shaders..."
python3 "$REPO_ROOT/scripts/blender/lightning/export_glsl.py" --repo-root "$REPO_ROOT" --out "$SOURCE_DIR/shaders"
...
if wheel_is_stale; then
    ...
    bash "$REPO_ROOT/scripts/collect_wheels.sh" --crate thyllore-effect-core --wheels-dir blender_addon/effects/lightning/wheels
fi

STAGE_DIR="$REPO_ROOT/build/blender_lightning_addon_stage_${PLATFORM}_${BUILD_MODE}"
rm -rf "$STAGE_DIR"
mkdir -p "$STAGE_DIR"
...
for required in shaders/lightning_resolve.glsl shaders/lightning_resolve.bindings.json common/coordinates.py common/effect_properties.py blender_manifest.toml; do
