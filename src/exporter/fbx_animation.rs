...
fn write_definitions<W: Write + Seek>(
    ...
    let total = 1 + model_count + node_attribute_count + 1 + 1 + curve_node_count + curve_count;
...
pub(crate) fn write_bone_model<W: Write + Seek>(
    ...
    attrs.append_string_direct(subclass)?;
    drop(attrs);
