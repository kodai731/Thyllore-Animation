...
pub fn curve_editor_capture_buffer(
    ...
    for &prop in visible_curves {
        ...
        buffer.snapshots.insert((bone_id, prop), points);
    }
}

pub fn curve_editor_swap_buffer(
    buffer: &mut CurveEditorBuffer,
    clip: &mut EditableAnimationClip,
    bone_id: BoneId,
) {
    let keys_to_swap: Vec<(BoneId, PropertyType)> = buffer
        .snapshots
        .keys()
    ...
    for (_, prop) in &keys_to_swap {
        ...
        if let Some(snapshot) = buffer.snapshots.get_mut(&(bone_id, *prop)) {
