...
pub(super) fn collect_selected_mesh_ids(world: &World, assets: &AssetStorage) -> Vec<u32> {
    let hierarchy_state = world.resource::<HierarchyState>();
    let mut selected_ids = Vec::new();

    for &entity in hierarchy_state.multi_selection.iter() {
        if let Some(mesh_ref) = world.get_component::<MeshRef>(entity) {
            if let Some(mesh_asset) = assets.get_mesh(mesh_ref.mesh_asset_id) {
