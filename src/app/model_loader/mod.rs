...
use crate::app::raytracing::scene_build;
...
pub(crate) unsafe fn append_model_to_scene(
    ...
    let procedural_primitives = scene_build::collect_procedural_primitives(world);
    let mesh_transforms = crate::ecs::systems::collect_mesh_transforms(world, assets);
    scene_build::rebuild_acceleration_structures(
        instance,
        device,
        command_pool,
        graphics,
    ...
    scene_build::update_ray_query_descriptor(device, raytracing)?;

    {
        let mut billboard = world.resource_mut::<BillboardData>();
        scene_build::update_billboard_descriptor(device, swapchain, &mut billboard)?;
    }
