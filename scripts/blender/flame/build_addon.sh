...
SOURCE_DIR="$REPO_ROOT/blender_addon/effects/flame"
...
Build the Thyllore Flame extension ZIP for the requested platform.
...
Options:
  --build-mode MODE     release (default) strips the debug/ package
                        (viewport dump, EXR sequence render, compositor setup);
                        debug keeps it and names the ZIP thyllore_flame_debug-*
...
case "$BUILD_MODE" in
    release) ZIP_BASENAME="thyllore_flame" ;;
    debug)   ZIP_BASENAME="thyllore_flame_debug" ;;
...
log() { echo "[flame/build_addon] $*"; }

log "Platform: $PLATFORM -> Blender: $BLENDER_NAME, mode: $BUILD_MODE, version: $VERSION"

log "Exporting flame GLSL shaders..."
python3 "$REPO_ROOT/scripts/blender/flame/export_glsl.py" --repo-root "$REPO_ROOT" --out "$SOURCE_DIR/shaders"

if ! ls "$SOURCE_DIR/wheels/thyllore_effect_core-"*.whl >/dev/null 2>&1; then
    log "Collecting wheels..."
    bash "$REPO_ROOT/scripts/collect_wheels.sh" --crate thyllore-effect-core --wheels-dir blender_addon/effects/flame/wheels
fi

STAGE_DIR="$REPO_ROOT/build/blender_flame_addon_stage_${PLATFORM}_${BUILD_MODE}"
rm -rf "$STAGE_DIR"
mkdir -p "$STAGE_DIR"
...
stage_sources() {
    ...
    rsync -a "${exclude_args[@]}" --exclude='.gitignore' --exclude='*.pyc' "$SOURCE_DIR/" "$STAGE_DIR/"
    rsync -a --exclude='__pycache__/' --exclude='*.pyc' "$COMMON_DIR/" "$STAGE_DIR/common/"
}
stage_sources
