...
4. `run_batch_capture_phase` (`src/app/features/batch_capture.rs`): only when the batch schedule asked
   for a capture this frame; waits idle, runs the `batch_capture!` hooks and saves the screenshot.
