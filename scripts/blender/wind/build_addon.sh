...
SOURCE_DIR="$REPO_ROOT/blender_addon/effects/wind"
...
Build the Thyllore Wind extension ZIP for the requested platform.
...
Options:
  --build-mode MODE     release (default) strips the debug/ package;
                        debug keeps it and names the ZIP thyllore_wind_debug-*
...
case "$BUILD_MODE" in
    release) ZIP_BASENAME="thyllore_wind" ;;
    debug)   ZIP_BASENAME="thyllore_wind_debug" ;;
...
STAGE_DIR="$REPO_ROOT/build/blender_wind_addon_stage_${PLATFORM}_${BUILD_MODE}"
rm -rf "$STAGE_DIR"
mkdir -p "$STAGE_DIR"
