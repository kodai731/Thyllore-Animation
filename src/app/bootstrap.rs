...
pub fn apply_engine_overrides(app: &mut App, overrides: &EngineCliOverrides) {
    ...
    if let Some(debug_view) = overrides.water_debug_view {
        app.data
            .ecs_world
            .resource_mut::<crate::ecs::resource::WaterRenderSettings>()
            .debug_view = debug_view;
    }
    ...
    if let Some(path) = overrides.flame_dump_path.clone() {
        app.data
            .ecs_world
    ...
    if let Some(n) = overrides.flame_count {
        if n >= 2 {
            for i in 1..n {
                ...
                let entity = crate::ecs::systems::spawn_flame_with_clip(
    ...
    if let Some(ref _scene_path) = overrides.scene_path {
        let model_state = app
            .data
            .ecs_world
            .resource::<crate::ecs::resource::ModelState>();
    ...
    if let Some(pixel) = overrides.pick_pixel {
        app.data
            .ecs_world
            .insert_resource(crate::ecs::resource::BatchPickRequest::new(pixel));
    }
...
pub unsafe fn init_flame_sdf_texture(app: &mut App, overrides: &EngineCliOverrides) -> Result<()> {
        ...
        flame_descriptor.update_image_views(
            ...
            FlameImageBindings {
                ...
                scene_depth_view: app
                    .resource::<crate::vulkanr::context::RenderTargets>()
                    .render
...
pub unsafe fn init_imgui_rendering(
    app: &mut App,
    system: &mut crate::platform::System,
) -> Result<()> {
