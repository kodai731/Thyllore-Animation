...
ENTRY_SHADER = "lightning/resolveFragment.frag"
...
def export_shader(entry: str, repo_root: str) -> tuple[list[str], dict]:
    expanded_lines = expand_includes(entry, repo_root)
    stripped_lines = strip_include_guards(expanded_lines)
    return convert_to_blender_dialect(stripped_lines)
...
def main() -> None:
    ...
    resolve_lines, resolve_bindings = export_shader(ENTRY_SHADER, repo_root)
