...
def insert_wheels_to_sys_path() -> None:
    ...
    if conflict_modules:
        print(
            f"[Thyllore Wind] WARNING: modules already imported by another addon — "
    ...
    for wheel_path in sorted(wheels_dir.glob("*.whl")):
        ...
        try:
            ...
            print(
                f"[Thyllore Wind] WARNING: failed to extract {wheel_path.name}: {e}"
            )
            continue
    ...
    if inserted > 0:
        ...
        print(
            f"[Thyllore Wind] Inserted {inserted} extracted wheels into sys.path"
        )
